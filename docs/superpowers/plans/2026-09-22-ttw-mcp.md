...
Expected: девять строк вывода; сайт медленный, скрипт займёт около минуты.
Проверить: `ls tests/fixtures/` показывает 9 файлов.
...
Expected: PASS, 10 тестов. Если `test_capped_search_fixture_has_500_rows`
...
- Produces: `clean(text)`, `parse_date(text)`, `parse_number(text)`,
  `parse_int(text)`, `parse_win_loss(text)`, `parse_score(raw)`,
  `extract_id(href)`, `require(node, selector, parser)`, `text_of(node)` —
  все импортируются из `ttw_mcp.parsers.common`
...
### Поправка после публикации: горизонта в 12 месяцев не существует

План и спека утверждали, что сайт отдаёт лишь последние 12 месяцев. Это
неверно: вывод был сделан по единственному игроку, который просто не играл
раньше. Перепроверка на девяти игроках дала до 94 недельных периодов и
историю с октября 2023 года. За горизонт был принят потолок в 30, которым
ограничены `summary["rated_periods"]` и ajax-эндпоинт графика.

Подробности и таблица замеров — в разделе «Горизонт данных не ограничен
окном» файла `docs/superpowers/specs/2026-09-22-ttw-mcp-design.md`.
Дальнейший текст плана оставлен как исторический документ и содержит
исходное ошибочное утверждение.

### Task 11: финальные правки по сквозному ревью

Единственная волна исправлений после финального ревью всей ветки. Полная
спецификация с кодом лежит отдельно, потому что затрагивает файлы восьми
предыдущих задач сразу:

`.superpowers/sdd/2026-09-22-ttw-mcp/task-11-brief.md`

Две критические находки — поиск игроков не отличал «нет таких» от
«сайт изменился», и все ошибки проекта стирались SDK до безликой строки
прежде, чем их увидит модель, — плюс шесть важных и четыре отложенных.
