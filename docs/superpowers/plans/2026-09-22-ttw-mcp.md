...
**Tech Stack:** Python 3.11+, `mcp[cli]` (FastMCP, транспорт stdio),
...
- Python `>=3.11`. Зависимости только: `mcp[cli]`, `httpx`,
...
| `ttw_mcp/server.py` | пять инструментов FastMCP |
...
- Produces: `mcp` (экземпляр `FastMCP`), функции `search_players`,
