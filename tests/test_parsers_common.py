...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("ЛЕВАЯ", ("ЛЕВАЯ", None)),
        ("Фамилия Имя Отчество (левой рукой)", ("Фамилия Имя Отчество", "левая")),
        ("Фамилия Имя Отчество (правой рукой)", ("Фамилия Имя Отчество", "правая")),
        ("Фамилия Лев Отчество", ("Фамилия Лев Отчество", None)),
    ],
)
def test_split_hand_handles_every_measured_form(raw, expected):
    # Больше четырнадцати форм замерены на именах из фикстур, включая
    # косвенный падеж («левой рукой») — прежний замер искал кандидатов по
    # списку «левая|правая» и был слеп к нему. Случай со «шипами» проверяет,
    # что остаток заметки не выбрасывается вместе с рукой, а «Фамилия Лев
    # Отчество» — что имя «Лев» не принимается за пометку.
