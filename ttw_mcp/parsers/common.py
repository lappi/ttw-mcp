...
_HAND = re.compile(
    r"\b(левая|правая|левой|правой)\b(?:\s*/\s*|\s+)?(?:рука|руки|рукой|руков)?\b",
    re.IGNORECASE,
)
_HAND_CANONICAL = {
    "левая": "левая",
...
def split_hand(name: str) -> tuple[str, str | None]:
    ...
    идентификатором и помечает это прямо в имени — больше чем четырнадцатью
    разными способами на выборке в тысячи имён: в скобках и без, с заглавной
    и строчной, в конце имени и приклеенной к фамилии, со словом «рука» и с
    опечаткой «руков», в именительном падеже («левая», «правая») и в
    косвенном, творительном («левой рукой», «правой рукой»). Прежний замер
    искал кандидатов только по списку «левая|правая» и был слеп к
    косвенному падежу: тот проходил мимо и застревал внутри имени вместе со
    скобкой, а opponent_hand у такого матча выглядел как «пометки нет»,
    хотя пометка была — просто не опознана. Теперь обе формы распознаются,
    а результат всегда приводится к именительному падежу, чтобы в ответе не
    завелось третье написание одной и той же пометки. Оставлять пометку
    внутри имени значит мешать сравнение имён и скрывать от потребителя,
    что игрок выступает нерабочей рукой с существенно другим рейтингом.
    ...
    if not cleaned:
        return name, None
    return cleaned, _HAND_CANONICAL[match.group(1).lower()]
