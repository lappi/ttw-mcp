...
def annotate_matches(profile: dict) -> None:
    ...
    Само проставляемое значение player_rating_at_match может оказаться
    None на неоднозначные даты — обе причины (история усечена; в этот день
    у игрока два турнира без известного порядка) описаны в докстринге
    rating_at_event, здесь не дублируются.
    """
